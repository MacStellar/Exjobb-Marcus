...
class P2PApiTest {
    ...
    @Test
    fun `Should return 400 if invalid P2P session is provided`() {
        val response =
            testRestTemplate.exchange(
                RequestEntity.get("/truid/v1/peer-to-peer/unknown_id").build(),
                String::class.java,
            )

        assertEquals(400, response.statusCode.value()) // Gör att den skickar 400 ist
    }

    @Nested
    inner class User1InitiatingSignup {
        // Ha failcases för saker innan rätt cookie blir initialized
        ...
        @Test
        fun `Should return '403 Forbidden' if no authorization is provided before creating P2P session `() {
            val response =
                testRestTemplate.exchange(
        ...
        @Test
        fun `Should return '403 Forbidden' if wrong state is provided`() {
            val (cookie, state) =
                testRestTemplate.exchange(
                    RequestEntity.get("/truid/v1/peer-to-peer").build(),
                    String::class.java,
                ).let {
                    val cookie = HttpCookie.parse(it.setCookie()).single()

                    assertEquals(true, cookie.isHttpOnly)
                    assertEquals(302, it.statusCode.value())
                    val urlOne = URIBuilder(it.headers.location!!)
                    val state = urlOne.getParam("state")!!
                    Pair(cookie, state)
                }
        ...
        @Nested
        inner class User1ConfirmedSignup {
            // Fixa så att användaren har fått en cookie här och sedan kör failcase för när dem har autentisierat
            // och försöker komma in i /create sidan. Alltså ha massa fel state och sånt
            ...
            @BeforeEach
            fun `Setup Truid PresentationResponse mock`() {
            ...
            inner class User1CompletedSignup {
                ...
                // Flytta upp initiate cookie hit
                lateinit var state: String

                // Ändra ksk så att det inte är Pair och let här sen
                @BeforeEach
                fun `Setup Authorization for user 1`() {
                    ...
                    // Flytta upp detta anrop till nästa nivå, lägg failcases på denna nivå
                    testP2PSessionId =
                        testRestTemplate.exchange(
                            RequestEntity.get("/truid/v1/peer-to-peer/create?code=1234&state=$state")
                                .header(HttpHeaders.COOKIE, cookie.toString())
                ...
                fun `It should return the status INITIATED`() {
                    ...
                    assertEquals(200, response.statusCode.value())
                    assertEquals("INITIALIZED", response.body)
                }
                ...
                inner class User2Flow {
                    ...
                    @Test
                    fun `It should return 404 if invalid P2P session is trying to be joined`() {
                        val response =
                            testRestTemplate.exchange(
                    ...
                    inner class User2CompletedSignup {
                            ...
                            val response =
                                testRestTemplate.exchange(
                                    RequestEntity.get("/truid/v1/peer-to-peer/join?code=1234&state=$state")
                                        .header(HttpHeaders.COOKIE, cookie.toString()).accept(MediaType.TEXT_HTML)
                                        .build(),
                                    String::class.java,
                                )
                        ...
                        @Nested
                        inner class User3Flow {
                            lateinit var newCookie: HttpCookie

                            @BeforeEach
                            fun `Setup Truid token endpoint-, presentation- mock`() {
                                println("kör denna först 1")
                                val response =
                                    mapOf(
                                        "refresh_token" to "refresh-token-2",
                                        "access_token" to "access-token-2",
                            ...
                            @BeforeEach
                            fun `Setup user 3 login`() {
                                println("kör denna först 2")
                                val responseOne =
                                    testRestTemplate.exchange(
                                        RequestEntity.get("/truid/v1/peer-to-peer").build(),
                                        String::class.java,
                                ...
                                // Checks whether the cookie is received and is set to httpOnly
                                newCookie = HttpCookie.parse(responseOne.setCookie()).single()

                                val responseTwo =
                                    testRestTemplate.exchange(
                                        RequestEntity.get("/truid/v1/peer-to-peer/$testP2PSessionId/init-join")
                                            .header(HttpHeaders.COOKIE, newCookie.toString()).build(),
                                        String::class.java,
                                    )
                                val url = URIBuilder(responseTwo.headers.location!!)
                                state = url.getParam("state")!!

                                testRestTemplate.exchange(
                                    RequestEntity.get("/truid/v1/peer-to-peer/join?code=1234&state=$state")
                                        .header(HttpHeaders.COOKIE, newCookie.toString()).accept(MediaType.TEXT_HTML)
                                        .build(),
                                    String::class.java,
                                )
                            }
